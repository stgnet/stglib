...
// TEMPLATE FOR NEW GROUP OF CLASSES
// use: change all instances of md5 to match new file name (stmd5.h)
...
// include other required stglib components here

// helpful notification on win32 compilers
#pragma message("using stmd5.h")


// Modifications to http://sourceforge.net/projects/libmd5-rfc
// by StG to fit into stglib class model
...
public:
	...
	{
		MD5Update(&mdContext,data,size);
		return(size);
	}
	virtual StSize _WriteTo(StBase &Destination)
	{
		MD5Final(&mdContext);
		Destination._Write(mdContext.digest,sizeof(mdContext.digest));
		return(0);
	}
